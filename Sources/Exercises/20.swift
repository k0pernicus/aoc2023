...
class Day20 : Day {
    ...
    enum Module {
        ...
        internal mutating func propagate(pulse: Pulse,
                                         ...
                                         states: [Module],
                                         pulsePerModule: [String: Pulse]) -> Pulse? {
            switch self {
            ...
            case let .conjunction(label: label):
                for module in conjunctionMapping[label]! {
                    if pulsePerModule[module] == .low { return .high }
                }
                return .low
            }
        }
    ...
    internal func part01(fromContent: String) throws -> Output01 {
        ...
        var pulsePerModule: [String: Pulse] = [:]
        ...
        for line in lines {
            ...
            case "%": inModule.removeFirst(); sModule = .flipflop(label: inModule, on: false); pulsePerModule[inModule] = .low
            ...
            while !propagationValues.isEmpty {
                ...
                for (index, var module) in states.enumerated() {
                    if module.label == moduleName {
                        let newPulse = module.propagate(pulse: pulse, conjunctionMapping: conjunctionMapping, states: states, pulsePerModule: pulsePerModule)
                        states[index] = module
                        if newPulse == nil {
                            // if let _ = pulsePerModule[moduleName] { pulsePerModule[moduleName] = pulse }
                            continue
                        }
                        if let _ = pulsePerModule[moduleName] { pulsePerModule[moduleName] = newPulse }
                        for dest in circuit[moduleName]! {
                            print("\(moduleName) \(newPulse!)-> \(dest)")
